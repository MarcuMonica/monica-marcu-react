...
afiseaza in consola un array in care fiecare consoana
este scrisa cu litera mare (vocalele nu)
`);
...
pozitie contine propozitia “Diferenta de varsa dintre {friendName} si
{personName} este {diff}”
`);
...
console.warn(`
Folosind metoda map pe arrayul skills, returneaza un array care
...
console.warn(`
Folosind metoda map pe arrayul skills, returneaza un array care contine cuvintele
inversate (exemplu: lmth)
`);
...
console.warn(`
Folosind metoda map pe arrayul friends, returneaza un
array care contine numele inversat al prietenilor pe
fiecare pozitie (exemplu: Stevenson Steven)
`);
...
console.warn(`
Folosind metoda map pe arrayul friends, returneaza un
array care contine pe fiecare pozitie diferenta dintre
lungimea totala al numelui complet (fara spatii) si
varsta prietenului de pe iteratie
`);
...
const arr9 = person.friends.map(({ name, surname, age }) => {
  ...
  return Math.abs(fullNameLength - age);
});
console.log(arr9);

console.warn(`
Folosind metoda map pe arrayul skills returneaza un
array care contine diferenta dintre lungimea fiecarui
skill si varsta prietenului
`);
const arr10 = person.skills.map((skill) => {
  const skillLength = skill.length;

  return person.friends.map(({ age }) => {
    return skillLength - age;
  });
});
console.log(arr10);
